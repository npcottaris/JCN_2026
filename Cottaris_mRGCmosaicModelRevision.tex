...
\textbf{C:} Achieved and target densities of mRGC RF centers along the horizontal meridian (green disks and white dashed line). 
...
%
\begin{itemize}
    \item \textbf{Stage 3A: computation of visual space--referred cone mosaic responses to stimuli used to measure vSTFs in macaque mRGCs}.  We begin by computing cone mosaic responses to gratings of different spatial frequencies delivered to the retina via human physiological optics \cite{Polans2015_PhysiologicalOptics}, as a proxy of how macaque optics blur the stimuli. This step is necessary for us to use the \emph{in vivo} data of Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}, which were collected with stimuli viewed through the animal's natural optics.
% 
    \item \textbf{Stage 3B: optimization of surround cone pooling functions for a subset of target synthetic mRGCs}. We choose a set of target cells that span the extent of the synthesized mRGC mosaic, and optimize the surround cone pooling functions for each. This optimization is done so that the ensuing target cells have (a) vSTF characteristics that are well approximated by a Difference of Gaussians (DoG) model, (b) have fitted DoG model parameters reasonably matching those reported by Croner \& Kaplan for eccentricities corresponding to the eccentricity of the target cells, and (c) have surround cone pooling weights that maintain macaque H1-like spatial properties as characterized by Packer \& Dacey.
    %
...
Croner \& Kaplan reported summaries of spatial RF characteristics across populations of mRGCs at different eccentricities, which were derived by fitting a Difference of Gaussian (DoG) model to the measured vSTFs, and tabulated the variation of the DoG parameters with eccentricity. 
The DoG model defined in spatial frequency, $\omega$, domain is given by:
\begin{equation}
DOG(\omega) = K_c \cdot R_c^2 \cdot \exp{ [-\pi \cdot R_c \cdot \omega]^2 } - K_s \cdot R_s^2 \cdot \exp{ [-\pi \cdot R_s \cdot \omega]^2 }
\end{equation}
...
The vSTF of a typical macaque mRGC is depicted in  Fig.~\ref{Fig:SurroundConnectivityOverview}A with cyan disks. The solid heavy line depicts the fitted DoG model, with the component center and surround vSTFs depicted by the thin solid and dashed lines, respectively.
%
%
\begin{figure}[!ht]
\centering
...
%
\textbf{A:} Typical macaque mRGC vSTF (cyan disks) fitted with a Difference of Gaussians (DoG) model (thick black line). The component center and surround vSTFs are depicted by the thin black and the dashed lines, respectively.
%
\textbf{B1 \& B2:} Ratios of surround to center characteristic radii, $R_s/R_c$, and ratios of surround to center integrated sensitivities, $(K_s/K_c \times R_s^2/R_c^2)$ as a function of eccentricity in the population of mRGCs recorded by Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}. The dashed lines represent the trends in these two ratios as a function of eccentricity. The cyan disks depict the ratios for the example vSTF depicted in A.
%
\textbf{C:} Depiction of the iterative estimation of surround cone pooling weights by forward simulation of the Croner \& Kaplan vSTF measurements.  See description in text.
}
\label{Fig:SurroundConnectivityOverview}
\end{figure}
%
%
The shape of the vSTF is determined by two key measures, the ratio of surround to center characteristic radii, $R_s/R_c$, and the ratio of surround to center integrated sensitivities, $K_s/K_c \times (R_s/R_c)^2$. The distributions of these two ratios as a function of eccentricity in the population of mRGCs recorded by Croner \& Kaplan are depicted by the gray squares in Figs~\ref{Fig:SurroundConnectivityOverview}B1 \& \ref{Fig:SurroundConnectivityOverview}B2. The mean variation in these two ratios, shown as dashed lines, are the target values used to derive the surround cone pooling weights in the synthetic mRGCs.
%
%

The optimization process is illustrated schematically in Fig.~\ref{Fig:SurroundConnectivityOverview}C. The vSTF of the target synthetic mRGC is computed by forward simulation of the experiment of Croner \& Kaplan. The time courses of responses of L-- and M--cones from the input cone mosaic to a drifting grating stimulus of spatial frequency $\omega$, $R_{\tiny \mbox{cones}}(\omega,t)$, (computed in Stage 3A) are depicted by the red and green traces in the rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C. A spatially weighted sum of these cone responses using the RF center cone pooling weights (computed in Stage 2), is used to compute the response of the RF center, $R_{\tiny \mbox{center}}(\omega,t)$. This operation, which is depicted by the red computation arm in Fig.~\ref{Fig:SurroundConnectivityOverview}C, is fixed throughout the optimization of the surround.
...
\begin{equation}
W_s(r) = K_{\mathrm{wide}} \times \exp \left [ -r/R_{\mathrm{wide}} \right] + K_{\mathrm{narrow}} \times \exp \left [ -r/R_{\mathrm{narrow}} \right]
\end{equation}
...
Beginning with a random initial value for the parameters of the double exponential distribution, we compute an initial estimate of the surround cone weights by evaluating $W_s(r)$ at the vicinity of the input cone mosaic that surrounds the RF center. These weights are depicted in the top-left circular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C (labeled as $i = 1$, with $i$ denoting iteration). Using these initial weights we compute a weighted sum of the surround cone responses to derive the initial estimate of the surround response, $R_{\tiny \mbox{surround}}(\omega,t)$ This operation is depicted by the blue computation arm in Fig.~\ref{Fig:SurroundConnectivityOverview}C.
...
\begin{equation}
R_{\tiny {\mbox{mRGC}}}(\omega, t) = R_{\tiny {\mbox{center}}}(\omega, t) - R_{\tiny {\mbox{surround}}}(\omega, t)
\end{equation}
\noindent
The amplitude modulation of $R_{\tiny {\mbox{mRGC}}}(\omega, t)$ is taken as the value of the vSTF at the examined spatial frequency, $STF_{\tiny {\mbox{mRGC}}}(\omega)$. Repeating over a range of spatial frequencies, we obtain the initial estimate of the full vSTF, which is depicted by the gray disks in the top-right rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C, labeled as $i = 1$. 

Following the experimental procedure of Croner \& Kaplan, we fit the computed $STF_{\tiny {\mbox{mRGC}}}(\omega)$ with a DoG model. This fit is depicted by the solid gray line in the top-right rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C. Note that the DoG model fit is constrained so that its shape parameters, $R_s/R_c$, and $K_s/K_c \times R_s^2/R_c^2$, both lie within a narrow range of the mean values of $R_s/R_c$, and $K_s/K_c \times R_s^2/R_c^2$ ratios reported for macaque mRGCs at corresponding eccentricities \cite{CronerKaplan1995_MandPcells}. At the first iteration, the  residual between the computed vSTF and the DoG model fit to it, is large.

This residual, depicted by the yellow bar in the right-most panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C, serves as an error signal. The optimization algorithm minimizes this error signal by adjusting the parameters of $W_s(r)$, which controls the surround weights, with the additional constraint that the parameters of $W_s(r)$ remain within a range of the values reported in macaque H1 horizontal cells \cite{PackerDacey2002_H1cellSpatialRFs}. 

The optimized surround cone pooling function is obtained when the residual between the computed and fitted vSTFs reaches a minimum value, labeled as $i = N$, in Fig.~\ref{Fig:SurroundConnectivityOverview}C. Additional details about this surround optimization method are provided in Supplemental Section \ref{Section:SupplementalMethods_Stage3B}.
%
%
%
%  -------- METHODS (STAGE 3C) -------
...
Since the current version of the mRGC model does not contain a temporal component, the response of the $k^{th}$-- mRGC to some stimulus at time instant, $t$, $R_{\mathrm{stim}}(k,t)$, is computed instantaneously from the response of the input cone mosaic to that stimulus at time $t$, $\mathbf{C}_{\mathrm{stim}}(:,t)$,  as follows:
%
\begin{equation}
\begin{split}
R_{\mathrm{stim}}(k,t) & = \displaystyle \frac{1}{\displaystyle \sum_{i=1} \mathbf{W}_{\mathrm{center}}(i,k)} \times  \dots \\
...
%
\begin{equation}
\mathbf{C}_{\mathrm{stim}}(:,t) = \displaystyle \frac{\mathbf{E}_{\mathrm{stim}}(:,t) - \mathbf{E}_{\mathrm{bkgnd}}(:)}{\mathbf{E}_{\mathrm{bkgnd}}(:)}
\end{equation}
%
...
\caption{
\textbf{Spatial characteristics of mRGC surround cone pooling inferred from DoG model fits to retinal space--referred STFs are not accurate.} The correspondence between actual and inferred surround cone pooling is examined for exemplar synthetic mRGCs at four eccentricities.
%
...
To estimate contrast sensitivity, we varied, for each tested spatial frequency, $\omega$, the contrast of the test stimulus and identified threshold contrast, $C_{\mathrm{threshold}}(\omega)$, as that for which the probability of correctly identifying the test versus a zero contrast stimulus was 80.6\%. Contrast sensitivity was defined as $CSF(\omega) = 1/C_{\mathrm{threshold}}(\omega)$. Estimates of so computed contrast sensitivities at three eccentricities are depicted in Fig.~\ref{Fig:Results_SpatioChromaticContrastSensitivity}.
%
%
\begin{figure}[!ht]
\centering
...
\caption{
\textbf{Computational observer spatial CSFs}\\
Left column depicts CSFs computed with stimuli that are delivered to the retina through typical human optics. Right column depict CSFs computed with stimuli that are delivered to the retina under diffraction limited optics. 
...
%
%
\subsection{Relation to previous modeling approaches}
Linear models of midget RGCs have been developed by others. The recent paper by Somaratna and Freeman \cite{2025_RFconstructionOfMRGCsInPrimateRetina} employs a cascade of spatial and temporal filters to model the spatial filtering effect of optics, the temporal filtering by cones, and spatiotemporal filtering by horizontal, bipolar and ganglion cells. This model is defined analytically as a function of spatial and temporal frequency.

 , both function-inspired, modeling the retinal circuitry as a cascade of linear spatio-temporal filters \cite{}  with and biophysics-inspired models. 


Function-inspired models attempt to model RGCs by employing the typical linear--non-linear--Poisson (LNP) standard model \cite{Carandini2005_DoWeKnowWhatTheEarlyVisualSystemDoes} using three components: spatial and temporal linear filtering by the RF center and surround mechanisms, followed by a gain control non-linearity that operates on the linear sum of responses of the center and surround mechanisms, and finally a stochastic spike generation mechanism. Biophysically-inspired models on the other hand, attempt to model RGCs by modeling the complexity of synaptic and cellular properties of cells at different retinal layers (cones, horizontal, bipolar, amacrine, and ganglion cells) and the connectivity between these layers \cite{Hennig2002_InfluenceOfRetinalSubcircuitsOnRGCnonLinearity}. And th\cite{Ly2025_VirtualHumanRetina}.
...
The need to mix data across the two closely related species produces tension in cases where the parameters for the two species differ.
...
In the second step, we only allow for cone swaps between an mRGC RF center and one of its neighbors. For each mRGC of the targeted input cone numerosity, we determine its 6 closest neighbors, but now without regard to their input cone numerosity. For each of these neighboring mRGCs, we evaluate the cost function, $\mathbf{C}$, for all possible combinations of cones from the target mRGC and cones from the neighboring mRGC and pick the combination that minimizes $\mathbf{C}$. The selected cone swap is executed only if the post-swap value of $\mathbf{C}$ is lower than its pre-swap value. Multiple passes through the entire mRGC mosaic, are executed, with each pass targeting mRGCs with progressively higher input cone numerosity.

The cost function, $\mathbf{C}$, employed to determine the optimal transfer/swap is based on the position and types of the cones pooled by the target mRGC, $t$, and the examined neighboring mRGC, $t_i$. For each examined pair of mRGCs, $(t,t_i)$, $\mathbf{C}^{(t,t_i)}$ is defined as:
%
\begin{equation}
\mathbf{C}^{(t,t_i)} = \displaystyle \phi \cdot \mathbf{C}^{(t,t_i)}_{\chi} + (1-\phi) \cdot \mathbf{C}^{t, t_i}_{\lambda}
\label{eq:tradeoffCostFunction}
\end{equation}
%
\noindent
where $\mathbf{C}^{(t,t_i)}_{\chi}$ is a spatial compactness cost component, $\mathbf{C}^{(t,t_i)}_{\lambda}$
...
The spatial compactness cost component, $\mathbf{C}^{(t,t_i)}_{\chi}$, is defined as: 
\begin{equation}
\mathbf{C}^{(t,t_i)}_{\chi} = \mathbf{C}^{(t, t_i)}_{\chi_{N}} + \mathbf{C}^{(t, t_i)}_{\chi_{o}}
\label{eq:spatialCompactnessCostComponent}
\end{equation}
\noindent 

In Eq.~\ref{eq:spatialCompactnessCostComponent}, $\mathbf{C}^{(t, t_i)}_{\chi_{N}}$ quantifies the differential input cone numerosity between the examined pair of mRGCs, and is defined as:
%
\begin{equation}
\mathbf{C}^{(t, t_i)}_{\chi_{N}} = \displaystyle \left | \left( N^t_L + N^t_M \right) - \left (N^{t_i}_L + N^{t_i}_M \right)  \right |
...
The $\mathbf{C}^{(t, t_i)}_{\chi_{o}}$ component of Eq.~\ref{eq:spatialCompactnessCostComponent} is a spatial overlap cost component, defined as the inverse of the distance between the centroids, $(P^t, P^{t_i})$, of the cones pooled by the two mRGCs, normalized by the sum of their spatial standard deviations, $(\sigma^t, \sigma^{t_i})$:
%
\begin{equation}
\mathbf{C}^{(t, t_i)}_{\chi_{o} } = \displaystyle 1 / \left( \frac{||P^t-P^{t_i}||}{\sigma^t+\sigma^{t_i}} \right)
\label{eq:spatialCompactnessOverlapCostTerm}
\end{equation}
%
A low value of $\mathbf{C}^{(t, t_i)}_{\chi_{o}}$ indicates low overlap between the sets of cones pooled by the examined pair of mRGCs and conversely, a high value indicates a large overlap.

The $\mathbf{C}^{t,t_i}_{\lambda}$ term, in Eq.~\ref{eq:tradeoffCostFunction}, is a cost component quantifying the sum of spectral impurities of the pair of analyzed mRGCs:
%
\begin{equation}
\mathbf{C}^{t,t_i}_{\lambda} = \displaystyle \mathbf{C}^t_{\lambda} + \mathbf{C}^{t_i}_{\lambda}
\label{eq:spectralPurityCostFunction}
\end{equation}
%
\noindent
The spectral impurity, $\mathbf{C}^t_{\lambda}$, quantifies the degree of non-specificity, with regard to the type of cone, in the pooling withing the RF center of an mRGC, and is defined as:
%
\begin{equation}
\mathbf{C}^t_{\lambda} = \displaystyle \frac{\min \left( \left [N^t_L, N^t_M \right ] \right) }{N^t_L + N^t_M}
\label{eq:spectralImpurityScore}
\end{equation}
%
\noindent 
Values of $\mathbf{C}^t_{\lambda}$ near zero indicate a low amount of mixture of L-- and M--cones, and therefore a RF with a high degree of spectral purity, and conversely, values of $\mathbf{C}^t_{\lambda}$, near 0.5, indicate an equal mixture of L-- and M--cones, and therefore a RF center with a low degree of spectral purity.  
...
The joint manipulation of the tolerance values applied to the parameters of the  $\mathrm{DOG}$ model fit to the vSTF, and to the parameters of the double exponential surround pooling model, $W_s(r)$,  allows for different options for deriving spatial pooling functions in synthetic mRGC surrounds. 

One option is to set very strict tolerances on the parameters of DoG model fit to the achieved vSTF, while allowing for a large tolerance in the parameters of $W_s(r)$. Results of this choice are depicted in the left-most column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. A second option would be to allow medium tolerance levels in both the DoG model fit and the $W_s(r)$. Results of this choice are depicted in the middle column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. A third option would be to enforce strict tolerances in $W_s(r)$, for example matching parameters of individual H1 horizontal cells, while allowing for a loose tolerance in the DoG model fit. Results of this choice are depicted in the right column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. In the present work, we chose the second option.
%
%
% FIGURE 15
%
...
Results from three options for the constraints are depicted. In the left column, we see results from a tight tolerance in the parameters of the DoG model fit to the vSTF and a loose tolerance in the parameters of the double exponential surround pooling model, $W_s(r)$. In the middle column, we see results from medium tolerance in both sets of parameters. In the right column, we see results from a loose tolerance in the DoG parameters and a tight tolerance in the $W_s(r)$ parameters. 
%
...
%
\textbf{C1-C3 :} $\mathrm{vSTF}$ computed under the three strategies are depicted by the. disks, and the $\mathrm{DOG}$ model fit by the gray lines, with the red and blue lines depicting the center and surround components of the $\mathrm{DOG}$ model fit. 
%
\textbf{D1-D3 :} Blue and orange bars depict the residuals for the ratios of visual space - referred $R_s/Rc$ and  $K_s/K_c \times \left(R_s/R_c\right)^2$ ratios. Black circles connected by a black line depict the enforced tolerance range in these ratios. The enforced tolerance value in D3 was $\tau = 0.5$, and is not visualized. Yellow bars depict the $||\mathrm{vSTF}(\omega) - \mathrm{DOG}(\omega)||$ residual.
}
