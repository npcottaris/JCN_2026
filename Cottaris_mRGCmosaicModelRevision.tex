...
Here, we describe an extension of ISETBio to model the mosaic of a class of retinal ganglion cells (RGCs), the midget RGC mosaic. RGCs are the only pathway for information transmission from the retina to the brain, and their properties surely impact visual performance on many tasks. The signals sent from the one million RGC axons that form the human optic nerve represent the signals from roughly 6.5 million cones and 110 million rods \cite{Osterberg1935_TopographyOfConesAndRodsInHumanRetina, Polyak1941_TheRetina}. Of these RGCs, mRGCs are a particularly important subtype, comprising 80\% of the perifoveal RGCs and 45\% of the peripherial RGCs. In the very central fovea, it has been estimated that the mRGCs are 95\% of the RGC population \cite{Dacey1993_MosaicOfMidgetRGCsInHumanRetina}.
...
The extant ISETBio model computes the mosaic of cone excitations. The model mRGCs are obtained by connecting their RFs to the cone mosaic. The connectivity matrix is constrained by anatomy and optimized through forward simulation of physiological measurements, so that the synthetic mRGCs are consistent with data across the visual field.
}
\label{Fig:Approach}
\end{figure}
%
...
A key challenge is the scarcity of \emph{in vitro} physiological data across the visual field which could be used to directly determine the wiring between the two mosaics. To address this, our algorithm primarily leverages more widely available \emph{in vivo} data for the optimization, while validating the model against \emph{in vitro} data where it exists. The resulting model is simultaneously consistent with cone light encoding, anatomical properties (including those of mRGCs and H1 horizontal cells), and both \emph{in vitro} and \emph{in vivo} physiological data. This makes the model versatile for simulating visual stimulation under \emph{in vivo}, \emph{in vitro}, and adaptive optics paradigms.


\subsubsection{Relationship to previous computational models of RGCs}
To our knowledge, no previous model of RGCs has attempted to realistically capture the effects of the front end encoding in the visual system, specifically the eccentricity and wavelength-varying nature of physiological optics, and the eccentricity-varying spatio-chromatic properties of the cone mosaic. Instead previous models of RGCs have either completely ignored the influence of optics \cite{Wohrer2009_VirtualRetina, Ly2025_VirtualHumanRetina}, or employed very simplistic models of the optics \cite{Freeman2025_RFconstructionOfMRGCsInPrimateRetina}. In addition, previous RGC models were designed to either operate directly the visual space, completely bypassing spatio-spectral filtering by the tri-chromatic cone mosaic \cite{Wohrer2009_VirtualRetina}, or have employed simplistic implementations of the cone mosaic \cite{Freeman2025_RFconstructionOfMRGCsInPrimateRetina}. Finally, none of the previous models are constructed to operate on stimuli defined in physical units of their spatial-spectral radiance as they are designed to operate on intensity-varying stimuli \cite{Freeman2025_RFconstructionOfMRGCsInPrimateRetina, Wohrer2009_VirtualRetina}. As such, previous models can not capture the rich spatio-chromatic interactions between stimuli and physiological optics, and how their combined effect shape  RGC responses. Indeed, we have recently shown that this interaction can have profound effects of the response properties of midget ganglion cells \cite{Cottaris2024_mRGCs_ConeSelectiveOrNot}. 

On the other hand, previous computational models of RGCs have focused on other, also important, components of the RGC circuit, that our linear spatio-chromatic model does not currently address, such as the impact of processing by retinal interneurons \cite{ Hennig2002_InfluenceOfRetinalSubcircuitsOnRGCnonLinearity, Wohrer2009_VirtualRetina, Freeman2025_RFconstructionOfMRGCsInPrimateRetina, Ly2025_VirtualHumanRetina} temporal dynamics \cite{ Hennig2002_InfluenceOfRetinalSubcircuitsOnRGCnonLinearity, Wohrer2009_VirtualRetina, Freeman2025_RFconstructionOfMRGCsInPrimateRetina, Ly2025_VirtualHumanRetina}, contrast grain control \cite{Pillow2005_PredictionDecodingRGCresponsesProbabilisticSpikingModel, Wohrer2009_VirtualRetina}, and spike generation \cite{Pillow2005_PredictionDecodingRGCresponsesProbabilisticSpikingModel, Wohrer2009_VirtualRetina, Ly2025_VirtualHumanRetina}. We plan to extend our linear spatiochromatic model of mRGCs to include several of these components as we describe in section \ref{Section:Extensions}.
...
\begin{itemize}
    ...
    \item In section \ref{Section:Results} we present, validate, and discuss a first application of the model. Specifically, we illustrate examples of synthesized mRGC mosaics (section \ref{Section:Results_MosaicsOfSynthesizedMRGCmosaics}), confirm that the model mRGC spatial RFs are consistent with \emph{in vivo} (section \ref{Section:Results_SynthesizedMRGCsConsistentWithInVivoPhysiology}), and \emph{in vitro} data (section \ref{Section:Results_SynthesizedMRGCsConsistentWithInVitroPhysiology}), demonstrate the significant impact of physiological optics (section \ref{Section:Results_ImpactOfPhysiologicalOpticsInMRGCSpatialRFs}), how simpler Difference-of-Gaussians models can fail to capture the true surround pooling (section \ref{Section:Results_DoGmodelFailure}), and finally we present first applications of the developed model, which illustrate how the model can be used to estimate the contribution of the mRGC mosaic to spatiochromatic contrast sensitivity across the visual field (section \ref{Section:applications}).

    \item In section \ref{Section:Discussion}, we summarize our work, present some of the model's applications, the model's current limitations, and planned expansions.
\end{itemize}
%
%
%
...
%
%
The synthesis of mRGC RF mosaics occurs in three stages. In the first stage, we generate spatial lattices representing the RF centers of the mosaic of mRGCs and the position of cones of the cone mosaic that provides input to the mRGC mosaic. In the second stage, we connect the input cone mosaic to the RF centers of the mRGC mosaic. In the third stage, we connect the input cone mosaic to the RF surrounds of the mRGC mosaic.
%
%
%
%  -------- METHODS (STAGE 1) -------
...
%
We begin by generating a lattice that represents the $(x,y)$ positions of mRGC RF centers.  This process comprises three sub-stages illustrated in Fig. ~\ref{Fig:LatticeGenerationOverview}.
%
%
\begin{figure}[!ht]
\centering
...
\textbf{D \& E:} Examples of $2^{\circ} \times 1^{\circ}$ mosaics of mRGC RF centers at eccentricities of 0$^\circ$ and 20$^\circ$, respectively, along the temporal meridian.
\textbf{F:} Achieved and target densities of cones along the horizontal meridian (purple disks and white dashed line, respectively).}
\label{Fig:LatticeGenerationOverview}
\end{figure}
%
%
\begin{itemize}
    \item \textbf{Stage 1A}. We estimate the mRGC RF center densities along the four principal meridians (0$^\circ$, 90$^\circ$, 180$^\circ$, and 270$^\circ$). These estimates are based on human data \cite{Watson2014_RGCmodel, CurcioAllen1990_RGCdensityHumanRetina}. We take the ON-mRGC density to be half of the total mRGC density, ignoring the possible density differences between ON-- and OFF--center mRGCs. The meridian functions are depicted in Fig.~\ref{Fig:LatticeGenerationOverview}A.
    %
    \item \textbf{Stage 1B}. We generate a continuous, two-dimensional map representing the mRGC RF density map, depicted in Fig. ~\ref{Fig:LatticeGenerationOverview}B. This map is created by linearly interpolating the meridian estimates, and it serves as a target for the algorithm in the next stage.
    %
    \item \textbf{Stage 1C}. We synthesize a sampling lattice that represents the mRGC RF center positions. The lattice is created using the iterative algorithm that we introduced earlier \cite{Cottaris2019_Optics_ConeMosaic_InferenceEngine} for generating cone mosaics, replacing the two-dimensional cone density map with the target mRGC RF density map. A typical lattice of mRGC RF positions is obtained after about 1,300 iterations and has a density that varies smoothly over space, matching the target density, as illustrated in Fig. ~\ref{Fig:LatticeGenerationOverview}C. Example patches of mRGC RF center mosaics synthesized at eccentricities of 0$^{\circ}$ and 20$^{\circ}$ along the temporal horizontal meridian, are depicted in Figs.~\ref{Fig:LatticeGenerationOverview}D \& E, respectively.
\end{itemize}

The same procedure is used to generate a lattice that represents the $(x,y)$ positions of cones, but, in this case, using the meridian densities of cone photoreceptors in human retina \cite{Curcio1990_HumanConeMosaic} as targets. The density of cones in the synthesized cone lattice also varies smoothly over space and matches closely the target cone density, as illustrated in Fig. ~\ref{Fig:LatticeGenerationOverview}F.
%
%
%
%  -------- METHODS (STAGE 2) -------
...
%
The connections between cones and mRGC centers are constrained by (1) anatomical data across the retina, specifically, the ratio of densities of mRGC RF centers to cones \cite{Watson2014_RGCmodel}, and (2) \emph{in-vitro} physiological data from peripheral retina, that (a) indicate that, unlike OFF-center mRGCs, which draw indiscriminately from all three cone types \cite{Klug2003_SconeOffMidget, Field2010_FunctionalConnectivitySingleCones, Patterson2019_SconeCircuitEdgeDetection}, ON-center mRGCs draw only from L- and M-cones, and (b) quantify the degree of RF center overlap in neighboring mRGCs \cite{Gauthier2009_ON_OFF_Overlap}. In the present version of the model, we only model ON-center mRGCs. The cone to mRGC RF center connectivity is established in 3 sub-stages, summarized here. 
%
 \begin{itemize}
    \item \textbf{Stage 2A}. In the first substage, each L- and M-cone gets connected to a single mRGC RF center; single RF centers can receive input from more than one cone. At this substage, each connected cone has unit connection weight. S-cones are not connected because they do not contribute to ON-center mRGCs.  This initial cone-to-RF center connectivity often results in inhomogeneities in the composition of neighboring mRGCs RF centers, which are dealt with in the next stage.  Algorithmic details regarding this substage are provided in Supplemental Section \ref{Section:SupplementalMethods_Stage2A}.
    %
    \item \textbf{Stage 2B}. This stage refines the center connections to establish a balance between the spectral purity and spatial compactness of the mRGC RF centers, which is quantified by a single parameter, $\phi$. For the body of this work, all mRGC mosaics are generated by maximizing spatial compactness, but the option to maximize spectral purity allows testing of different scenarios where mRGC RF centers may be biased to some extent towards cone type selective pooling \cite{Field2010_FunctionalConnectivitySingleCones,Wool2018_NonSelectingWiringMidgetRGC}. At this substage, connected cones retain their unit connection weights. Algorithmic details regarding this substage are provided in Supplemental Section \ref{Section:SupplementalMethods_Stage2B}.
    %
    \item \textbf{Stage 2C} - Finally, the mutual exclusivity constraint enforced in stages 2A and 2B is lifted, and single cones are permitted to connect to multiple nearby mRGC RF centers. The extent of divergence varies with retinal eccentricity, being minimal in the fovea and increasing towards the periphery to match experimental observations \cite{Gauthier2009_ON_OFF_Overlap}. This is done by varying the exponent of a supra-Gaussian distribution that describes the spatial weighting profile of cone connections to the RF centers which at this substage become non-binary. Algorithmic details regarding this substage are provided in Supplemental Section \ref{Section:SupplementalMethods_Stage2C}.
\end{itemize}
%
...
Example mosaics of RF centers synthesized at four eccentricities along the temporal horizontal meridian at the end of stage 2A are depicted in Fig. ~\ref{Fig:Results_CenterConnectedMosaicsNoOverlap}, where each green ellipse represents the spatial extent of the RF center of a single mRGC. 
%
%
\begin{figure}[!ht]
\centering
...
%
%
As mentioned previously, stage 2B allows for different optimizations of cone pooling within the mRGC RF centers, which is controlled by the spatial compactness/spectral purity tradeoff parameter, $\phi$. Fig. ~\ref{Fig:Results_SpatialSpectralTradeoff} depicts examples of mRGC RF center mosaics all synthesized at a single eccentricity (12$^\circ$), but under different values of $\phi$.
%
%
\begin{figure}[!ht]
\centering
...
Insets in A and C depict pooling of cones within the RF centers of the two mRGC RF centers contained within the yellow square. The inset in C illustrates how RF center overlap and spatial jitter is introduced as the algorithm avoids cones that are close to the RF center in order to maximize spectral purity within RF centers.}
\label{Fig:Results_SpatialSpectralTradeoff}
\end{figure}
%
%
The mosaic synthesized under $\phi = 1$, where spatial compactness is maximal and spectral purity constraint is not enforced, is depicted in Fig. ~\ref{Fig:Results_SpatialSpectralTradeoff}A. Note that the RF centers tile the visual field relatively uniformly with no overlap. Figures~\ref{Fig:Results_SpatialSpectralTradeoff}B and \ref{Fig:Results_SpatialSpectralTradeoff}C depict mosaics synthesized as $\phi$ decreases to 0.5 and 0.0, respectively, which increasingly enforces center connections to cones of the same type. Note that this occurs at the cost of spatial compactness with the RF centers becoming spatially disordered and overlapping to a greater extent.

By varying $\phi$ we can examine the effect that cone-selective pooling may have on mRGC RF spatial structure, as well as on the spatio-chromatic processing in the mRGC pathway. Current electrophysiological evidence favors little selective cone pooling within peripheral RF centers of mRGCs, i.e., a $\phi$ value of $\approx 1$ \cite{Field2010_FunctionalConnectivitySingleCones, Kling2020_BioRxiv_FunctionalOrganizationOfMidgetAndParasolRGCs, Wool2018_NonSelectingWiringMidgetRGC}. However, the degree of cone type selectivity in more central locations is not known with as much certainty. For example, there is anatomical evidence that ON-center mRGCs in the fovea contact multiple ON-cone bipolars, as opposed to OFF-center mRGCs, which contact single OFF-cone bipolars \cite{Kolb2003_TheMidgetPathwaysInPrimateRetina}, and also electrophysiological evidence that the RFs of parafoveal mRGCs appear to be pooling from more than one cones \cite{McMahon2000_FineStructureParvocellularRFs}. 

The question of whether foveal mRGCs that pool from more than one cone in the RF centers are doing so selectively remains unanswered. Our modeling approach allows exploration of the benefits and tradeoffs of cone-selective pooling at any retinal eccentricity, although we do not pursue such exploration in this paper.
%
%
%
%
...
In the final sub-stage of establishing the mRGC RF center wiring to the cone mosaic, the mutual exclusivity constraint is lifted and single cones are permitted to connect to multiple nearby mRGC RF centers. This divergence of cone connections is enabled by replacing the binary distribution of cone pooling weights in the mRGC RF centers with a supra Gaussian distribution, as illustrated in Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}. 
%
%
\begin{figure}[!ht]
\centering
%\includegraphics[width=0.7\textwidth]{figure5.png}
\includegraphics[width=0.82\textwidth]{figure5.pdf}
\caption{
\textbf{Mosaics of mRGC RF centers with divergent cone connections (stage 2C).} \\
\textbf{A:} Variation of supra Gaussian exponent with eccentricity. The exponent is set to 10 in the central retina, resulting in flat top weight distribution with zero overlap (gray histograms). As eccentricity is increased, the exponent is gradually decreased, achieving a value of 2.0, at around 15$^\circ$ (red histograms).
%
...
\textbf{C, D, \& E: } Mosaics at 10$^\circ$, 13$^\circ$, and 18$^\circ$, along the temporal horizontal meridian with divergent cone connections. The RF center ellipses encompass the ensemble of cones with pooling weights $\ge$ 0.1. 
...
%
%
Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}A depicts how a progressively increasing overlap in neighboring mRGC RF centers with eccentricity, is accomplished by varying the exponent of the supra-Gaussian distribution. In central retina, the exponent is kept at 10, which results in a flat top distribution of weights with minimal overlap between neighboring RF centers (gray histograms in the inset of Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}A). As eccentricity increases beyond 7$^\circ$, the exponent decreases, reaching a value of 2 (Gaussian) at around 15$^\circ$, which results in Gaussian distributions of weights with significant overlap between neighboring RF centers (red histograms in the inset of Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}A). 

To our knowledge, there is no physiological data on the variation with eccentricity of the divergence of cone connections to nearby mRGC RF centers. Therefore the varying, with eccentricity, super Gaussian distribution of cone weights is an arbitrary mechanism. It's intent is to capture the fact that in the fovea, input to mRGC RF centers comes exclusively or mostly \cite{Kolb2003_TheMidgetPathwaysInPrimateRetina, McMahon2000_FineStructureParvocellularRFs} from a single cone, whereas in the periphery, \emph{in vitro} measurements reveal that neighboring mRGC RF centers abut at approximately one standard deviation of their Gaussian RF profile \cite{Gauthier2009_ON_OFF_Overlap}. 

The transformation of cone pooling weights from binary and mutually exclusive to graduated and shared is depicted in Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}B for an mRGC located at an eccentricity of 12$^{\circ}$, with gray and blue histograms depicting spatial distributions of cone pooling weights in this cell before and after sub-stage 2C.
%
%
%

Figs~\ref{Fig:Results_CenterConnectedMosaicsOverlap}C--E depict mosaics with divergent connections synthesized at three eccentricities. In these mosaic depictions, each green ellipse represents the spatial extent that encompasses all cones that are pooled by the RF center of a single mRGC with weights $\ge 0.1$. For the mosaic centered at 10$^\circ$ (panel C), divergence of cone connections has just begun. The overlap in RF centers due to the divergence of connections increases as we move in eccentricity from 9$^\circ$ on the right side to 11$^\circ$, on the left side. For the mosaic centered at around 13$^\circ$ (panel D), cone divergence and RF center overlap is higher and again increases with increasing eccentricity. For the mosaic centered at around 18$^\circ$ (panel D), divergence of cone connections has assymptoted, and we have a constant RF center overlap. 

Finally, Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}F provides a visualization comparable to the visualization commonly reported by \emph{in vitro} RF mapping studies \cite{Gauthier2009_ON_OFF_Overlap}. It depicts the same mosaic as Fig.~\ref{Fig:Results_CenterConnectedMosaicsOverlap}C, but with ellipses encompassing cones that are pooled with weights $\ge e^{-1/2} \approx 0.67$. This depiction choice makes the overlap less visually salient.
%
%
%
%
...
In the last stage of mRGC mosaic synthesis, we derive the cone pooling weights for the mRGC RF surrounds. Since there are no clear anatomical data on surround sizes, these weights are determined using \textit{in vivo} characterizations of the visual space--referred spatial transfer function, $\mathbf{vSTF(\omega)}$, i.e., a cell's response amplitude as a function of spatial frequency, $\omega$, in macaque mRGCs \cite{CronerKaplan1995_MandPcells}, which are available as a function of eccentricity. 

We incorporate these data into the model using numerical optimization. More specifically, we determine the cone-to-mRGC RF surround connections such that a forward simulation of the \textit{in vivo} physiological experiments of of Croner \& Kaplan \cite{CronerKaplan1995_MandPcells} through the model best reproduces the experimental data. This approach allows us to use data collected through physiological optics, which blur the stimulus in an eccentricity and wavelength dependent manner, to determine the wiring of cones to mRGC RF surrounds. 

Importantly, the optimization is achieved while adhering to the already established connectivity between the cone mosaic and mRGC RF centers. Simultaneously, the parametric form of the surrounds is constrained based on Packer \& Dacey's characterizations of the spatial RF of macaque H1 horizontal cells \cite{PackerDacey2002_H1cellSpatialRFs}, which are the main components of the linear spatial mRGC RF surrounds \cite{Smith1992_RGCresponsesToRelativePhaseOfHeterochromaticallyModulatedLights}. The use of optimization around forward simulation of an experiment to integrate data from multiple non-commensurate sources is an important innovation of our RGC modeling approach. 

Stage 3 proceeds in three sub-stages. 
%
\begin{itemize}
    \item \textbf{Stage 3A: computation of visual space--referred cone mosaic responses to stimuli used to measure $\mathbf{vSTF}$s in macaque mRGCs}.  We begin by computing cone mosaic responses to gratings of different spatial frequencies delivered to the retina via human physiological optics \cite{Polans2015_PhysiologicalOptics}, as a proxy of how macaque optics blur the stimuli. This step is necessary for us to use the \emph{in vivo} data of Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}, which were collected with stimuli viewed through the animal's natural optics.
% 
    \item \textbf{Stage 3B: optimization of surround cone pooling functions for a subset of target synthetic mRGCs}. We choose a set of target cells that span the extent of the synthesized mRGC mosaic, and optimize the surround cone pooling functions for each. This optimization is done so that the ensuing target cells (a) have $\mathbf{vSTF}$ characteristics that are well approximated by a Difference of Gaussians model, with model parameters reasonably matching those reported by Croner \& Kaplan at corresponding eccentricities, and (b) have surround cone pooling weights that maintain macaque H1-like spatial properties as characterized by Packer \& Dacey.
    %
    \item \textbf{Stage 3C: computation of surround cone pooling weights for all cells in  the synthesized mRGC mosaic}. Once the surround cone pooling functions are derived for the target synthetic mRGCs, surround pooling weights for all cells in the synthesized mRGC mosaic are computed by interpolation. A small amount of jitter in the ratio of the surround to center weights is added to simulate the variance in integrated surround to center ratio at each eccentricity seen in the macaque data.
\end{itemize}
...
In these computations, we employ human physiological optics matched to the eccentricity of each synthesized mRGC, but we adjust the defocus term of the modeled optics so as to maximize the Strehl ratio (ratio of peak sensitivity of the PSF at the wavelength of focus, here 550 nm, to the peak sensitivity of a diffraction-limited PSF). This is done as a proxy to the experimental paradigm of Croner \& Kaplan, in which corrective lenses were used to maximize cell responses at high spatial frequencies (personal communication with the late Ehud Kaplan). 
%
%
%
%
...
Croner \& Kaplan reported summaries of spatial RF characteristics across populations of mRGCs at different eccentricities, which were derived by fitting a Difference of Gaussian model to the measured $\mathbf{vSTF}$, and tabulated the variation of the model's parameters with eccentricity. 
The $\mathbf{DoG}$ model defined in spatial frequency, $\omega$, domain is given by:
\begin{equation}
\mathbf{DoG}(\omega) = K_c \cdot R_c^2 \cdot \exp{ [-\pi \cdot R_c \cdot \omega]^2 } - K_s \cdot R_s^2 \cdot \exp{ [-\pi \cdot R_s \cdot \omega]^2 }
\end{equation}
where $K_c$ and $K_s$ are the peak sensitivities of the RF center and RF surround mechanisms, and $R_c$ and $R_s$ are the corresponding characteristic radii. 

The $\mathbf{vSTF}$ of a typical macaque mRGC is depicted in  Fig.~\ref{Fig:SurroundConnectivityOverview}A with cyan disks. The solid heavy line depicts the fitted model, with the component center and surround $\mathbf{vSTF}$s depicted by the thin solid and dashed lines, respectively.
%
%
\begin{figure}[!ht]
\centering
...
\caption{
\textbf{Derivation of cone weights to mRGC surrounds}.
%
\textbf{A:} Typical macaque mRGC $\mathbf{vSTF}$ (cyan disks) fitted with a Difference of Gaussians model (thick black line). The component center and surround $\mathbf{vSTF}$s are depicted by the thin black and the dashed lines, respectively.
%
\textbf{B1 \& B2:} Ratios of surround to center characteristic radii, $R_s/R_c$, and ratios of surround to center integrated sensitivities, $(K_s/K_c \times R_s^2/R_c^2)$ as a function of eccentricity in the population of mRGCs recorded by Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}. The dashed lines represent the trends in these two ratios as a function of eccentricity. The cyan disks depict the ratios for the example $\mathbf{vSTF}$ depicted in A.
%
\textbf{C:} Depiction of the iterative estimation of surround cone pooling weights by forward simulation of the Croner \& Kaplan $\mathbf{vSTF}$  measurements.  See description in text.
}
\label{Fig:SurroundConnectivityOverview}
\end{figure}
%
%
The shape of the $\mathbf{vSTF}$ is determined by two key measures, the ratio of surround to center characteristic radii, $R_s/R_c$, and the ratio of surround to center integrated sensitivities, $K_s/K_c \times (R_s/R_c)^2$. The distributions of these two ratios as a function of eccentricity in the population of mRGCs recorded by Croner \& Kaplan are depicted by the gray squares in Figs~\ref{Fig:SurroundConnectivityOverview}B1 \& \ref{Fig:SurroundConnectivityOverview}B2. The mean variation in these two ratios, shown as dashed lines, are the target values used to derive the surround cone pooling weights in the synthetic mRGCs.
%
%

The optimization process is illustrated schematically in Fig.~\ref{Fig:SurroundConnectivityOverview}C. The $\mathbf{vSTF}$ of the target synthetic mRGC is computed by forward simulation of the experiment of Croner \& Kaplan. The time courses of responses of L-- and M--cones from the input cone mosaic to a drifting grating stimulus of spatial frequency $\omega$, ${\mathrm R}_{\mathrm {cones}}(\omega,t)$, (computed in Stage 3A) are depicted by the red and green traces in the rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C. A spatially weighted sum of these cone responses using the RF center cone pooling weights (computed in Stage 2), is used to compute the response of the RF center, ${\mathrm R}_{\mathrm {center}}(\omega,t)$. This operation, which is depicted by the red computation arm in Fig.~\ref{Fig:SurroundConnectivityOverview}C, is fixed throughout the optimization of the surround.

In the computation of the spatial distribution of surround cone pooling weights, we impose a parametric form that is described by the sum of a narrow and a wide exponential components, based on characterizations of the spatial RF properties of H1 horizontal cells by Packer \& Dacey \cite{PackerDacey2002_H1cellSpatialRFs}:
\begin{equation}
{\mathbf W}_s(r) = K_{\mathrm{wide}} \times \exp \left [ -r/R_{\mathrm{wide}} \right] + K_{\mathrm{narrow}} \times \exp \left [ -r/R_{\mathrm{narrow}} \right]
\end{equation}
...
Beginning with a random initial value for the parameters of the double exponential distribution, we compute an initial estimate of the surround cone weights by evaluating $\mathbf W_s(r)$ at the vicinity of the input cone mosaic that surrounds the RF center. These weights are depicted in the top-left circular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C (labeled as $i = 1$, with $i$ denoting iteration). Using these initial weights we compute a weighted sum of the surround cone responses to derive the initial estimate of the surround response, $R_{\mathrm{surround}}(\omega,t)$ This operation is depicted by the blue computation arm in Fig.~\ref{Fig:SurroundConnectivityOverview}C.

The composite response of the synthesized mRGC is obtained by instantaneously subtracting the surround response from the center response, as follows: 
\begin{equation}
{\mathrm R}_{\mathrm{mRGC}}(\omega, t) = {\mathrm R}_{\mathrm{center}}(\omega, t) - {\mathrm R}_{\mathrm{surround}}(\omega, t)
\end{equation}
\noindent
The amplitude modulation of ${\mathrm R}_{\mathrm{mRGC}}(\omega, t)$ is taken as the value of the $\mathbf{vSTF}$ at the examined spatial frequency, $STF_{\mathrm{mRGC}}(\omega)$. Repeating over a range of spatial frequencies, we obtain the initial estimate of the full $\mathbf{vSTF}$, which is depicted by the gray disks in the top-right rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C, labeled as $i = 1$. 

Following the experimental procedure of Croner \& Kaplan, we fit the computed $STF_{\mathrm{mRGC}}(\omega)$ with a DoG model. This fit is depicted by the solid gray line in the top-right rectangular panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C. Note that the DoG model fit is constrained so that its shape parameters, $R_s/R_c$, and $K_s/K_c \times R_s^2/R_c^2$, both lie within a narrow range of the mean values of $R_s/R_c$, and $K_s/K_c \times R_s^2/R_c^2$ ratios reported for macaque mRGCs at corresponding eccentricities \cite{CronerKaplan1995_MandPcells}. At the first iteration, the  residual between the computed $\mathbf{vSTF}$ and the DoG model fit to it, is large.

This residual, depicted by the yellow bar in the right-most panel of Fig.~\ref{Fig:SurroundConnectivityOverview}C, serves as an error signal. The optimization algorithm minimizes this error signal by adjusting the parameters of $\mathbf W_s(r)$, which controls the surround weights, with the additional constraint that the parameters of $\mathbf W_s(r)$ remain within a range of the values reported in macaque H1 horizontal cells \cite{PackerDacey2002_H1cellSpatialRFs}. 

The optimized surround cone pooling function is obtained when the residual between the computed and fitted $\mathbf{vSTF}$s reaches a minimum value, labeled as $i = N$, in Fig.~\ref{Fig:SurroundConnectivityOverview}C. Additional details about this surround optimization method are provided in Supplemental Section \ref{Section:SupplementalMethods_Stage3B}.
%
%
%
%  -------- METHODS (STAGE 3C) -------
...
Once these $N_{xy} \times N_{c} \times 2$ surround cone pooling functions are derived, we compute surround cone pooling weights for all synthetic mRGCs. For each target mRGC we determine the 3 nearest spatial grid nodes, and extract the optimized surround cone functions that were derived at this node for the cone numerosity that matches that of the target mRGC, for both L- and M-center cone dominance variants. Then we evaluate the six optimized surround pooling functions at the input cone mosaic in the vicinity of the target mRGC, deriving six sets of surround cone pooling weights. The target cell's surround cone pooling weights are determined by interpolating the 6 sets of weights spatially, weighted inversely proportionally by the distance between the location of the target mRGC and the location of the optimized model, and spectrally, weighted based on the relative L-/M- cone weight ratio in the RF center of the target mRGC.
%
%
%
%
...
A fully synthesized mRGC mosaic consists of two connectivity matrices: $\mathbf{W}_{\mathrm{center}}(i,k)$, determined in synthesis stage 2, and $\mathbf{W}_{\mathrm{surround}}(i,k)$, determined in synthesis stage 3, which capture the weights by which the RF center and surround, respectively, of the $k^{th}$-- cell in the mRGC mosaic pools signals from the $i^{th}$ cone in the input cone mosaic.

Since the current version of the mRGC model does not contain a temporal component, the response of the $k^{th}$-- mRGC to some stimulus at time instant, $t$, $\mathbf R_{\mathrm{stim}}(k,t)$, is computed instantaneously from the response of the input cone mosaic to that stimulus at time $t$, $\mathbf{C}_{\mathrm{stim}}(:,t)$,  as follows:
%
\begin{equation}
\begin{split}
{\mathbf R}_{\mathrm{stim}}(k,t) & = \displaystyle \frac{1}{\displaystyle \sum_{i=1} \mathbf{W}_{\mathrm{center}}(i,k)} \times  \dots \\
&  \left( \sum_{i=1} \mathbf{W}_{\mathrm{center}}(i,k) \cdot \mathbf{C}_{\mathrm{stim}}\left(i, t \right) -
\sum_{j=1} \mathbf{W}_{\mathrm{surround}}(j,k) \cdot \mathbf{C}_{\mathrm{stim}} \left(j, t \right) \right )
\end{split}
\end{equation}
%

To mimic adaptation to the background stimulus, the mRGC mosaic typically operates on cone contrast responses, instead of cone excitation responses, so the $\mathbf{C}_{\mathrm{stim}}(:,t)$ term in the above equation is computed as follows:
%
\begin{equation}
\mathbf{C}_{\mathrm{stim}}(:,t) = \displaystyle \frac{\mathbf{E}_{\mathrm{stim}}(:,t) - \mathbf{E}_{\mathrm{bkgnd}}(:)}{\mathrm{E}_{\mathbf{bkgnd}}(:)}
\end{equation}
%
where $\mathbf{E}_{\mathrm{stim}}(:,t)$ is the cone mosaic excitation response to the examined stimulus at time $t$, and $\mathbf{E}_{\mathrm{bkgnd}}(:)$ is the cone mosaic excitation response to a uniform field, zero contrast stimulus, whose mean $(x,y)$ chromaticity and mean luminance match those of the examined stimulus.
%
%
%
%
...
\caption{
\textbf{Spatial RF characteristics of synthetic mRGCs} \textbf{A:} Mosaic of RF centers of an mRGC mosaic synthesized at 4.5$^\circ$ along the temporal horizontal meridian. \textbf{B1--B3:} Cone pooling maps for 3 exemplar cells whose positions within the mRGC mosaic are labeled in A. Pink and cyan disks depict L-- and M--cones, respectively, whose RF center pooling weights are $\ge 0.1$, or whose RF surround weights are $\ge 0.005$. Gray disks represent either S-cones, which are not pooled in our model, or L--/M--cones with pooling weights lower than the labeling thresholds. The solid and dashed black lines depict the extents of the RF center and surround pooling regions. \textbf{C1--C3:} Y-axis integrated cone pooling weight profiles within the RF center (maroon) and surround (slate). The dashed lines depict the visual space--referred line weighting functions as derived by fitting Difference of Gaussians (DoG) models to each cell's $\mathbf{vSTF}$. \textbf{D1--D3:} The $\mathbf{vSTF}$s of the exemplar mRGCs, computed under physiological optics, are depicted by the gray disks. The gray, maroon, and slate lines depict the DoG model fits to these $\mathbf{vSTF}$s, and the models' component center and surround $\mathbf{vSTF}$s, respectively.
}
...
The cell depicted in Fig.~\ref{Fig:Results_SynthesizingMRGCMosaics}B1 is located at an eccentricity of 3$^o$. Its RF center pools from a single L-cone and its RF surround pools from a total of 16 L-- and M--cones. The cell depicted in Fig.~\ref{Fig:Results_SynthesizingMRGCMosaics}B2, also located at 3$^o$, pools from two L--cones in its RF center, and its RF surround is larger, pooling from 44 L-- and M--cones.  The cell depicted in Fig.~\ref{Fig:Results_SynthesizingMRGCMosaics}B3 is located at 6$^o$. Its RF center, which pools from 2 L--cones and 1 M--cone, and its surround are both larger than those of the first 2 cells.

The cone pooling maps depicted in Figs~\ref{Fig:Results_SynthesizingMRGCMosaics}B1--B3 illustrate the spatial connectivity between the input cone mosaic and the center and surround subregions of mRGC RFs, but do not depict the strength of these connections. In this sense, these maps depict the type of information that is available from detailed anatomical studies. 

Figs~\ref{Fig:Results_SynthesizingMRGCMosaics}C1--C3 add to this view by providing information about the strength of the cone inputs for the three exemplar cells. Here, the maroon and slate histograms depict the cells' spatially integrated (along the y-axis) cone pooling weights for the RF center and the surround mechanisms, respectively. Note that in the cell depicted in Fig.~\ref{Fig:Results_SynthesizingMRGCMosaics}C1, the double exponential spatial profile of the surround cone pooling mechanism, with a sharp peak around the RF center and more shallow weights in peripheral regions, is prominent. In the two other cells shown, this feature is less prominent. 

This observation, where cells with larger RF centers have less peaked surround weights than cells with smaller RF centers is seen commonly in our synthetic mRGCs. The change in surround pooling characteristics results from constraints in the model, which maintain $\mathbf{vSTF}$ shape parameters that are consistent \emph{in vivo} measurements \cite{CronerKaplan1995_MandPcells} while at the same time remaining consistent with the surround parametric form indicated by measurements of H1 receptive fields \cite{PackerDacey2002_H1cellSpatialRFs}.

Visual space--referred spatial transfer functions are commonly measured in  \emph{in vivo} physiological assessments to estimate spatial RF properties of mRGCs \cite{CronerKaplan1995_MandPcells, Lee2012_mRGCsurroundConeSpecificity}. The $\mathbf{vSTF}$s computed for the three examined synthetic mRGCs are depicted by the gray disks in Figs~\ref{Fig:Results_SynthesizingMRGCMosaics}D1--\ref{Fig:Results_SynthesizingMRGCMosaics}D3. The corresponding DoG model fits are depicted by the solid gray lines, and the spatial RF profiles corresponding to these DoG model fits are depicted by the dashed lines in Figs~\ref{Fig:Results_SynthesizingMRGCMosaics}C1--C3. Contrasting these inferred spatial RF profiles with the actual cone pooling profiles, it becomes evident that one cannot use characterizations obtained under physiological optics viewing to directly infer the characteristics of spatial pooling of cone signals in the retina. We discuss this issue further in later sections.
%
%
%
...
%
%
To validate our model, we synthesized mRGC mosaics across a wide region of the retina, and computed $\mathbf{vSTF}$s of individual mRGCs by probing them with drifting achromatic gratings of different spatial frequencies delivered to the retina under human physiological optics appropriate for the eccentricity of the examined cells, simulating the experimental paradigm of Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}.

To compare synthetic and macaque mRGCs we fitted the synthetic cell $\mathbf{vSTF}$s with the DoG model employed by Croner \& Kaplan and compared the ratios of surround to center characteristic radii, $R_s/R_c$, and ratios of surround to center integrated sensitivities, $(K_s/K_c \times R_s^2/R_c^2)$, to those reported by Croner \& Kaplan \cite{CronerKaplan1995_MandPcells}. The results of this analysis are depicted in the left and right panels of Fig.~\ref{Fig:Results_ValidationAgainstCronerAndKaplan} for mRGC mosaics synthesized under the physiological optics of two different human observers. 
%
%
\begin{figure}[!ht]
\centering
...
%
\textbf{A: } Center and surround cone pooling weight maps for three synthetic mRGCs at an eccentricity of 6.75 mm along the temporal raphe. Solid and dashed contours include cones pooled by the RF center and RF surround, respectively, with pooling weights $>0.005 \times$ the peak center weight.
%
\textbf{B: } Center and surround cone pooling weights for three macaque mRGCs recorded in vitro at an eccentricity of 6.75 mm along the temporal raphe. White and black disks indicate cones pooled by the RF center and RF surround respectively, with same threshold pooling weights as in A. The macaque mRGCs are from the \emph{in vitro} recordings of Field \emph{et al.} \cite{Field2010_FunctionalConnectivitySingleCones}.
\textbf{C1 \&C2: } Comparison of minor and major center and surround diameters of the 3 macaque mRGC cells (depicted by the square, circle and star symbols) against the corresponding distributions in populations of synthetic mRGCs at eccentricities of 6.75 mm (C1) and 6.0 mm (C2).
}
\label{Fig:Results_ValidationAgaistField}
\end{figure}
%
%
Fig.~\ref{Fig:Results_ValidationAgaistField}A depicts the cone pooling maps of three synthetic mRGCs at a temporal eccentricity of 6.75 mm, and Fig.~\ref{Fig:Results_ValidationAgaistField}B depicts the spatial distribution of cone pooling weights measured by Field \emph{et al.} \cite{Field2010_FunctionalConnectivitySingleCones}) in three macaque mRGCs at the same eccentricity (adapted from their Fig. 4). The visualized surround cones have pooling weights $>0.005~\times $ the peak center cone weight for both the synthetic mRGCs and the macaque mRGCs (Greg Field, personal communication).
...
To further explore the relationship in center and surround cone pooling regions between synthetic and macaque mRGCs we contrasted the diameters of cone pooling regions of the three depicted macaque mRGCs against those of populations of synthetic mRGCs at two eccentricities: 6.75 mm, and 6.0 mm. Results of this analysis are depicted in Fig.~\ref{Fig:Results_ValidationAgaistField}C1 and Fig.~\ref{Fig:Results_ValidationAgaistField}C2. Here, the minor and major center and surround diameters of the 3 macaque mRGC cells are depicted by the yellow square, pink circle and magenta star symbols. The  corresponding distributions of minor and major center and surround cone pooling diameters in populations of synthetic mRGCs are depicted by the yellow, pink and magenta histograms, respectively. Note that at 6.75 mm (Fig.~\ref{Fig:Results_ValidationAgaistField}C1), the cone pooling regions of the synthetic mRGCs are consistently larger than those of the three macaque mRGCs. However, at the slightly less peripheral eccentricity of 6.0 mm (Fig.~\ref{Fig:Results_ValidationAgaistField}C2) a better agreement exists between model and macaque mRGCs.
...
%
\textbf{B:} Visual space--referred spatial RF maps of the same cells, computed under physiological optics appropriate for the cells' eccentricity.
%
\textbf{C:} Point spread functions of physiological optics at corresponding eccentricities.
}
...
\caption{
\textbf{Spatial characteristics of mRGC surround cone pooling inferred from DoG model fits to retinal space--referred $\mathbf{vSTF}$s are not accurate.} The correspondence between actual and inferred surround cone pooling is examined for exemplar synthetic mRGCs at four eccentricities.
%
\textbf{Left panels: } Cone input maps of exemplar synthetic mRGCs. 
%
\textbf{Middle panels:} Line weighting functions of cone inputs pooled by the RF center and surround mechanisms are depicted by pink and slate filled histograms, respectively. Dashed lines represent the line weighting functions inferred from DoG model fits to the model cells' retinal space--referred STFs.
%
\textbf{Right panels:} Retinal space--referred STFs, depicted by gray disks, are computed for stimuli delivered to the retina via diffraction limited optics. The DoG model fits to the computed STFs are depicted by the dashed black lines, with the corresponding center and surround component STFs depicted by the pink and slate lines, respectively.
}
...
%
%
The cone pooling maps of four exemplar mRGCs are depicted in the left column. The cells in the top two rows both have RF centers with a single cone input, whereas the cell in the third row has a 2-cone RF center, and the cell in the fourth row has a 3-cone RF center. The pink and maroon histograms depicted in the middle column of Fig.~\ref{Fig:Results_FailureOfDoGModel}, are the y-axis integrated cone pooling weights within the RF centers and surrounds, respectively, and the superimposed dashed lines depict the center and surround line weighting profiles, as estimated by fitting the DoG model to the cells' retinal space--referred STFs, which are depicted in the right column by the gray disks.

Note that although the fitted DoG models (solid lines) captures the computed retinal space--referred STFs accurately for all cells, the inferred spatial RF profile, depicted by dashed lines in the middle column, do not accurately capture the surround mechanism cone pooling regions, which are depicted by the slate histograms. The discrepancy between actual and inferred surround pooling is most obvious in the two top cells which have single-cone RF centers, and becomes less pronounced as the RF center size increases. The discrepancy involves both the spatial extent and the peak sensitivity of the inferred surround pooling, which is estimated by the DoG model to be more diffuse with a weaker peak sensitivity than the cell's actual surround cone pooling. 
...
%
%
We used a computational observer approach to compute spatial contrast sensitivity functions (CSFs) for achromatic and L-M cone opponent stimuli, based both on the representation at the cone mosaic and on the representation provided by our mRGC model. To do so, we computed responses to drifting gratings of varying spatial frequency, $\omega$. 

For the achromatic gratings, the L--, M-- and S--cone contrast component gratings were in phase, $C^L(\omega, x, y) = C^M(\omega, x, y) = C^S(\omega, x, y)$. For the $\mathrm{L-M}$ gratings, the L-- and M--cone contrast components were in antiphase, $C^L(\omega, x, y) = -C^M(\omega, x, y)$, and $C^S(\omega, x, y) = 0$. For all stimuli, mean $(xy)$ chromaticity was $(0.30, 0.32)$ and mean luminance was 100 $cd/m^2$. Stimuli were simulated as presented on a typical CRT monitor, but with 20-bit channel DACs, to avoid intrusion of quantization effects.
...
At present, our mRGC model does not include spike generation or response noise. Therefore, in these computations we modeled response variability by adding zero mean Gaussian noise to the noise-free model mRGC responses. This approximation allows us to examine relative sensitivity across stimuli and eccentricity, but the overall level of predicted sensitivity is arbitrary.
Given the choice of Gaussian noise, we used a template matching computational observer decision rule, with templates provided by the noise-free responses to the stimuli being discriminated.
...
To estimate contrast sensitivity, we varied, for each tested spatial frequency, $\omega$, the contrast of the test stimulus and identified threshold contrast, $\mathbf C_{\mathrm{threshold}}(\omega)$, as that for which the probability of correctly identifying the test versus a zero contrast stimulus was 80.6\%. Contrast sensitivity was defined as $\mathbf{CSF}(\omega) = 1/\mathbf{C}_{\mathrm{threshold}}(\omega)$. Estimates of so computed contrast sensitivities at three eccentricities are depicted in Fig.~\ref{Fig:Results_SpatioChromaticContrastSensitivity}.
%
%
\begin{figure}[!ht]
\centering
...
\caption{
\textbf{Computational observer spatial CSFs}. Left column depicts CSFs computed with stimuli that are delivered to the retina through typical human optics. Right column depict CSFs computed with stimuli that are delivered to the retina under diffraction limited optics. 
\textbf{A1:} CSFs for a $0.6^{\circ} \times 0.6^{\circ}$ foveal mRGC mosaic and of its input cone mosaic, depicted by disks and triangles, respectively. Gray: achromatic; pink: L-M. This mosaic contains 4628 mRGCs.
\textbf{A2:} Diffraction-limited CSFs of the same foveal mRGC mosaic. 
...
Additional observations are notable at 14$^{\circ}$ (Figs.~\ref{Fig:Results_SpatioChromaticContrastSensitivity}C1 and \ref{Fig:Results_SpatioChromaticContrastSensitivity}C2). First, a notch arises in the achromatic CSF at high spatial frequencies for the mRGC CSF that is not apparent in the cone CSF. This seems unlikely to be an optical effect, because it is more salient in Fig.~\ref{Fig:Results_SpatioChromaticContrastSensitivity}C2 where optical effects are not present. Additional computations (not shown), indicate this notch is orientation dependent and has to do with the precise alignment of individual cones with the receptive field of an mRGC. We do not explore it further here.  We do note that our computational observer is with respect to a noise level that makes it more sensitive than the human observer, so that the notch shown in Fig.~\ref{Fig:Results_SpatioChromaticContrastSensitivity}C1 would be unlikely to be revealed with psychophysics conducted with natural optics. Indeed, repeating these simulations (data not shown) with twice the noise variance, we observe, in addition to an overall reduction in sensitivity, that the notch disappears under both physiological and adaptive optics conditions. It is an interesting question as to whether this notch could be observed experimentally under adaptive optics conditions. 
...
As a second example application, we examined chromatic sensitivity for uniform fields modulated in different directions in the LM--cone contrast plane. We used the same computational observer approach described above, and evaluated threshold for stimuli whose contrast was modulated in time. The cone contrasts of stimuli at different chromatic directions, $\theta$, on the LM plane were:
\begin{center}
\[
\begin{array}{lcl}
C^L(\theta) & = & \rho \cdot \cos(\theta)\\
C^M(\theta) & = & \rho \cdot \sin(\theta)\\
C^S(\theta) & = & 0
\end{array}
\]
\end{center}
%
\noindent 
For each $\theta$, we varied $\rho$ to find its threshold value for discriminating that modulation direction from a zero contrast stimulus with a probability of 0.806. To summarize the computed thresholds across the different chromatic directions, we fit ellipses to the locus of threshold contrast points. Fig.~\ref{Fig:Results_ChromaticSensitivity} depicts computational observer thresholds for synthetic mRGC mosaics and for their input cone mosaics at different eccentricities. 
%
%
\begin{figure}[!ht]
\centering
...
How computational observer sensitivity changes with eccentricity depends on how stimulus size is covaried with eccentricity, as does human sensitivity (e.g. \cite{Hansen2009_ColorPerceptionIntermediatePeriphery}). Comparison of the magnitude of sensitivity for cone- and mRGC-based computational observers depends on how the noise levels are chosen. For these example calculations, we focus on the shape rather than magnitude of the elliptical threshold contours. Each contour shown in Fig.~\ref{Fig:Results_ChromaticSensitivity} is normalized so that the threshold along the M cone direction is equal to one.

The first observation is that the normalized contours for the cone-based observer are similar across eccentricities and align with the L- and M- cone contrast axes. They are more elongated in the M-direction because our mosaics have more L cones than M cones. The alignment with the axes is expected \cite{Stockman2010_ColorVisionMechanisms}, and the similarity of the normalized shapes occurs because this shape depends primarily on the relative numbers of L and M cones.

The contours for the mRGC-based computations, in contrast, change markedly with eccentricity. For the foveal mosaic, the threshold ellipse is highly elongated along $45^\circ$ in the cone-contrast diagram, indicating that the highest discrimination thresholds occur when  $c^L = c^M$ and lowest thresholds occur when $c^L = -c^M$. This difference in comparison to the cone-based computations is a consequence of the chromatic-opponency of the mRGC RFs in the fovea. These have single cone centers, and thus opponency between the centers and the surrounds as the surrounds draw on mixed cone-types \cite{Lennie1991_TheDesignOfChromaticallyOpponentRFs, Mullen96_LossesInPeripheralColorSensitivity}. The opponency leads to the observed contour orientation because it leads to cancellation of non-opponent L-- and M--cone signals for low spatial frequency stimuli \cite{Wandell1995_FoundationsOfVision, Stockman2010_ColorVisionMechanisms}. 

As eccentricity increases, the contours first become less elongated and then elongation starts increasing again but along the $135^\circ$ rather than the $45^\circ$ axis. This is because the cone non-selective wiring model we implemented leads to progressively less opponency with increasing RF center size \cite{Lennie1991_TheDesignOfChromaticallyOpponentRFs, Mullen96_LossesInPeripheralColorSensitivity, Wool2018_NonSelectingWiringMidgetRGC}.

Although the qualitative features that emerge from this example calculation are understood in the literature, the example illustrates that our model enables this type of calculation to be made quantitatively in a way that takes chromatic aberration, stimulus size and spatial frequency, and retinal position into account.  Of particular interest to us will be exploring how this type of threshold contour varies with the center wiring parameter $\phi$ of our model, which specifies the tradeoff between spatial homogeneity and spectral purity of mRGC RF centers.
%
%
%
%
...
Datasets (mRGCmosaics) generated during the current study are available at:  {\tt{https://github.com/isetbio/isetbio/tree/main}}\\
...
The code used to generate the data, and various tutorials on how to use the software are available at: {\tt{https://github.com/isetbio/isetbio/tree/main}}\\

\noindent
An introduction to using the  software is available at:\\
{\tt{https://github.com/isetbio/isetbio/wiki/Retinal-ganglion-cell-(RGC)-mosaics}}\\

\subsection*{Author contribution}
NPC: conceptualization, algorithm development, coding, data curation, validation, visualization, writing of original draft\\
...
During the first sub-stage of cone-mRGC RF center connectivity, cones are connected to single mRGC RF centers  based on the local topology of the 2 lattices. Starting with the cell whose RF center is at most central location of the mRGC lattice, we connect $n_{\mathrm pool}(\epsilon)$ number of L-- and M--cones to it,
%
\begin{equation}
\label{eq:conesPerRFcenter}
n_{\mathrm pool}(\epsilon) = \displaystyle \lfloor \frac{D_{\mathrm{cones}}(\epsilon)}{D_{\mathrm{mRGC RF}}(\epsilon)} \rfloor
\end{equation}
%
where $D_{\mathrm{cones}}(\epsilon)$ and $D_{\mathrm{mRGC RF}}(\epsilon)$ are the local spatial densities of the cone mosaic and of the mRGC RF centers, respectively, at the eccentricity, $\epsilon$, of the target mRGC.

We draw from the nearest $n_{\mathrm pool}(\epsilon)$, typically 6, cones that have not yet been connected and whose distance to the mRGC RF center does not exceed a fraction of the local mRGC RF center spacing. This fraction is a parameter of the model and for the work presented here was set to 0.6. 

Continuing with these assignments of cones to mRGC RF centers, we move outward to more peripheral locations in the mRGC RF mosaic, connecting cones to each mRGC center. Any L-- and M--cones that remain unconnected at the end of this sub-stage are then connected to their nearest mRGC center, so that all cones are connected to one mRGC center. 

This sub-stage can result in local inhomogeneities in both the number of cones and the type of cones pooled within neighboring RF centers. These inhomonegenities are smoothed out as part of the next sub-stage.
%
%
% --- STAGE 2B ----
%
...
In the second sub-stage of the RF center connectivity, convergent connections from multiple cones to single mRGC RF centers are optimized according to a desired balance between spatial and spectral inhomogeneities. This is achieved by reassigning cones between nearby mRGC RF centers, which itself occurs in two steps. 
...
%
\noindent
where $\mathrm{C}^{(t,t_i)}_{\chi}$ is a spatial compactness cost component, $\mathrm{C}^{(t,t_i)}_{\lambda}$
is a spectral purity cost component, and $\phi$ is a free parameter that controls the desired trade-off between spatial compactness and spectral purity of the RF centers.

The spatial compactness cost component, $\mathrm{C}^{(t,t_i)}_{\chi}$, is defined as: 
\begin{equation}
...
In Eq.~\ref{eq:spatialCompactnessCostComponent}, $\mathrm{C}^{(t, t_i)}_{\chi_{N}}$ quantifies the differential input cone numerosity between the examined pair of mRGCs, and is defined as:
%
\begin{equation}
...
\end{equation}
with $N^t_L$ and $N^t_M$ are the numbers of L-- and M--cones pooled by the RF center of mRGC $t$, respectively.

The $\mathrm{C}^{(t, t_i)}_{\chi_{o}}$ component of Eq.~\ref{eq:spatialCompactnessCostComponent} is a spatial overlap cost component, defined as the inverse of the distance between the centroids, $(P^t, P^{t_i})$, of the cones pooled by the two mRGCs, normalized by the sum of their spatial standard deviations, $(\sigma^t, \sigma^{t_i})$:
%
\begin{equation}
...
The $\mathrm{C}^{t,t_i}_{\lambda}$ term, in Eq.~\ref{eq:tradeoffCostFunction}, is a cost component quantifying the sum of spectral impurities of the pair of analyzed mRGCs:
%
\begin{equation}
...
%
\noindent
The spectral impurity, $\mathrm{C}^t_{\lambda}$, quantifies the degree of non-specificity, with regard to the type of cone, in the pooling withing the RF center of an mRGC, and is defined as:
%
\begin{equation}
...
The tradeoff parameter $\phi$ factor in Eq.~\ref{eq:tradeoffCostFunction}, controls how the optimal cone transfer/swap is determined. When $\phi = 1$, cone reassignments/swaps are selected so as to minimize the spatial compactness cost, when $\phi = 0$, cone reassignments are chosen so as to minimize the spectral purity cost, and for intermediate values of $\phi$, cone reassignments are chosen so as to minimize a ratio of the two costs. 
%
%
% --- STAGE 2C ----
%
...
We begin by fitting an ellipsoid to the spatial pooling map of cones that are exclusively connected to the RF center of an mRGC, and extracting the rotation, $\alpha$, and the major/minor axes, $\sigma_x, \sigma_y$ of the fitted ellipsoid. Next, a super-Gaussian ellipsoid, $G(\mathrm{x,y,n})$, defined as:
%
\begin{equation}
...
By varying the exponent of the super Gaussian, $n$, we model varying degrees of cone divergence. When $n = 10$, we obtain a flat-top Gaussian with very sharp fall-offs, modeling minimal cone divergence. When $n = 2$, we get a standard Gaussian modeling cone divergence that is consistent with the \emph{in-vitro} measurements of RF center overlap at peripheral locations. 

By varying $n$ with eccentricity using a sigmoidal function we obtain a gradual transition in cone divergence with eccentricity. The slope and mid- point of the sigmoidal variation of $n$ are currently chosen arbitrarily, with the only restrictions that above 15$^o$, $n$ is stable at 2.0, and below 7$^o$, $n$ is stable at 10.0. The weights of divergent cone-mRGC RF center connections are computed by evaluating the super-Gaussian ellipsoid at the positions of all cones in the vicinity of the examined mRGC. 
%
%
...
\end{equation}
\noindent 
This aspect of the optimization captures the observation that DoG models provide a reasonable fit to \emph{in-vivo} mRGC RFs. To ensure adherence to the \emph{in-vivo} data of Croner \& Kaplan, the $\mathrm{DOG}(\omega)$ model fit is constrained so that the ratio of surround to center radii, $R_s/R_c$, and the ratio of surround to center integrated sensitivities, $K_s/K_c \times \left(R_s/R_c\right)^2$, both remain within a specified tolerance value, $\tau = 0.15$, from the  mean values of the corresponding ratios for the Croner \& Kaplan population of macaque mRGCs at the eccentricity of the synthesized mRGC.
...
The joint manipulation of the tolerance values applied to the parameters of the  $\mathrm{DOG}$ model fit to the $\mathbf{vSTF}$, and to the parameters of the double exponential surround pooling model, $\mathbf W_s(r)$,  allows for different options for deriving spatial pooling functions in synthetic mRGC surrounds. 

One option is to set very strict tolerances on the parameters of DoG model fit to the achieved $\mathbf{vSTF}$, while allowing for a large tolerance in the parameters of $\mathbf W_s(r)$. Results of this choice are depicted in the left-most column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. A second option would be to allow medium tolerance levels in both the DoG model fit and the $\mathbf{W}_s(r)$. Results of this choice are depicted in the middle column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. A third option would be to enforce strict tolerances in $\mathbf W_s(r)$, for example matching parameters of individual H1 horizontal cells, while allowing for a loose tolerance in the DoG model fit. Results of this choice are depicted in the right column of Figure~\ref{Fig:Supplemental_SurroundOptimizationStrategies}. In the present work, we chose the second option.
%
%
% FIGURE 15
%
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{figure15.pdf}
\caption{
{\bf Effect of constraints on surround cone pooling.}\\
Results from three options for the constraints are depicted. In the left column, we see results from a tight tolerance in the parameters of the $\mathbf{DoG}$ model fit to the $\mathbf{vSTF}$ and a loose tolerance in the parameters of the double exponential surround pooling model, $\mathbf W_s(r)$. In the middle column, we see results from medium tolerance in both sets of parameters. In the right column, we see results from a loose tolerance in the DoG parameters and a tight tolerance in the $\mathbf W_s(r)$ parameters. 
%
\textbf{A1-A3 :} The tolerance range enforced during model fitting in the joint space of the two surround cone pooling related parameters, $V_{\mathrm{narrow}}/V_{\mathrm{wide}}$ and $R_{\mathrm{narrow}}/R_{\mathrm{wide}}$ for the three examined strategies is depicted by the yellow rectangles. Stars depict the corresponding parameter values in four macaque H1 horizontal cells from the study of Packer \& Dacey. The red disk depicts the achieved parameter values under each strategy for an example foveal synthetic mRGC. 
%
...
%
\textbf{C1-C3 :} $\mathbf{vSTF}$ computed under the three strategies are depicted by the gray disks, and the $\mathbf{DOG}$ model fit by the gray lines, with the red and blue lines depicting the center and surround components of the $\mathrm{DOG}$ model fit. 
%
\textbf{D1-D3 :} Blue and orange bars depict the residuals for the ratios of visual space - referred $R_s/Rc$ and  $K_s/K_c \times \left(R_s/R_c\right)^2$ ratios. Black circles connected by a black line depict the enforced tolerance range in these ratios. The enforced tolerance value in D3 was $\tau = 0.5$, and is not visualized. Yellow bars depict the $||\mathbf{vSTF}(\omega) - \mathbf{DOG}(\omega)||$ residual.
}
